...
//        Install_Handler(&readfds, NULL, NULL, 0.0);
        ...
        if (ctrl_login_skipped){
           if(ctrl_server_attach_prompt){
            ...
            if (fcntl(nvtsock, F_SETFL, O_NONBLOCK) == -1) {
              TTCN_error("fcntl(nvtsock, F_SETFL, O_NONBLOCK) failed: %d,%s",errno, strerror(errno));
            }
            return;
        }
...
void TELNETasp__PT::outgoing_send(const CHARSTRING& send_par)
{
  ...
  if (server_mode) {
    ...
    if(ctrl_server_attach_prompt){
     ...
     if  (send_nonblock(nvtsock, (const char*) (send_par), send_par.lengthof(), 0 ) < 0) {
         if(ctrl_server_failsafe_sending){
           TTCN_warning("TCP send failed");
         }
         else{
    ...
    if(ctrl_CRLF) send_nonblock( nvtsock, ( const char* ) (send_par+"\r\n"),
	send_par.lengthof()+2, 0 );
    else send_nonblock( nvtsock, ( const char* ) (send_par+"\n"),
	...
	log_debug( "%s: TELNETasp__PT::outgoing_send end", port_name);
}

int TELNETasp__PT::send_nonblock(int sockfd, const char *buf, size_t len, int flags){
  int ret_val=len;
  bool w=false;
  while(len){
    ...
    if(ret>0){
      ...
      if(errno==EAGAIN){
	      if(!w){
          TTCN_warning( "%s: TELNETasp__PT::send_nonblock, the send operation would block, waiting...", port_name);
          w=true;
        }
        dont_close_fd=sockfd;   // Signal the event handler, that ther is a pending write operation on the fd
                                // So the event handler won't close the conection.
        TTCN_Snapshot::block_for_sending((int)sockfd);
        if(dont_close_fd==-1){  // delayed close of the socket, the other side closed the socket meanwhile
                                // The event handler detected the close of the fd, but didn't called the close()
                                // Just set the dont_close_fd -1
          if (server_mode){
        ...
        dont_close_fd=-1;
        TTCN_warning( "%s: TELNETasp__PT::send_nonblock, next try...", port_name);
      } else {
          TTCN_warning( "%s: TELNETasp__PT::send_nonblock, fail... %d,%s ", port_name, errno, strerror(errno));
        return -1;
      }
    }
  }
  if(w){
    TTCN_warning( "%s: TELNETasp__PT::send_nonblock, the message finally sent", port_name);
  }
	log_debug( "%s: TELNETasp__PT::send_nonblock, data sent", port_name);
  return ret_val;
}
